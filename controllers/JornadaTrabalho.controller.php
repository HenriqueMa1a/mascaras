<?php
if(!class_exists('JornadaTrabalhoDAO')){ require_once 'models/JornadaTrabalhoDAO.class.php';}
...
class JornadaTrabalhoController extends Controller{
    ...
    public function __construct(){
        parent::__construct('cadastro-jornada-trabalho');
    }
    ...
    public function listar() { 
        ...
        try {
            ...
            $idJornada = self::getVar('idJornada');
            $descricao = self::getVar('descricao');

            $DAO = new JornadaTrabalhoDAO();
            $listagem = $DAO->listar($page, $rows, $sidx, $sord, $idJornada, $descricao);
            if ($listagem) {
                ...
                foreach ($listagem as $item){
                    $bt_view = '<a class="btn btn-vazio btn-sm btn-grid" title="Visualizar registro." onclick="visualizar('.$item->getIdJornada().');"><span class="glyphicon glyphicon-search"></span>';
                    $this->response->aaData[$i] = array($bt_view, $item->getIdJornada(), $item->getDescricao(), $item->getQtdMascaras());
                    $i++;
                }
    ...
    public function listarCombo() { 
	   ...
	   try {
			...
			$listagem = $DAO->listar(null, null, 'descricao', 'asc', $idJornada, $descricao);
			
			if ($listagem) {
                $this->response->records = count($listagem);
                $i = 0;
                foreach ($listagem as $item){
                    $this->response->rows[$i]['value'] = intval($item->getIdJornada()); 
    ...
    public function retornar() {
        ...
        try{
            ...
            if ($Id){
                ...
                $obj = $DAO->retornar($Id);
                if ($obj){
                    $this->response->Id = $obj->getIdJornada();
    ...
    public function incluir() {
        try {
            $Descricao = self::getVar('descricao');
            $QtdMascaras = self::getVar('qtdMascaras');
            ...
            if(!$QtdMascaras){
                $this->response->mensagem = "O campo <b>Quantidade de Máscaras</b> é de preenchimento obrigatório.";
                return false;
            }
            
            $obj = new JornadaTrabalho();
            $obj->setDescricao($Descricao);
            $obj->setQtdMascaras(intval($QtdMascaras));
            ...
            if ($DAO->salvar($obj)){
                ...
                $this->response->sucesso = 1;
                $this->response->mensagem = "Registro '$Descricao' foi cadastrado com sucesso.";
            } else {
                $this->response->mensagem = $DAO->getMensagem();
            }
        ...
        try {
            ...
            if ($Id){
                ...
                if ($obj){
                    ...
                    $obj->setDescricao($Descricao);
                    $obj->setQtdMascaras($QtdMascaras);
                    $obj->setIdUsuarioAcao($IdUsuarioAcao);

                    if ($DAO->atualizar($obj)){
                        $this->response->sucesso = 1;
                        $this->response->mensagem = "Registro '$Descricao' atualizado com sucesso.";
                    } else {
                        $this->response->erro = $DAO->getErro();
                        $this->response->mensagem = $DAO->getMensagem();
                    }
        ...
        try {
            ...
            if ($Id){
                ...
                if ($obj){
					...
					$DAO1 = new FuncionarioDAO();
					$obj1 = $DAO1->listar(null, null, 'nome', null, null, null, null, null, null, $Id, null);
					if ($obj1){
						$this->response->mensagem = "Registro não pode ser excluído, existem funcionários vinculados a esta jornada de trabalho<br>";
						$this->response->mensagem .= "Favor verificar o cadastro de funcionários.";
					} else {	
						$this->response->Descricao = $obj->getDescricao();
						if ($DAO->excluir($Id)){
							$this->response->sucesso = 1;
							$this->response->mensagem = "O registro <b>'".$obj->getDescricao()."'</b> foi removido com sucesso.";
						} else {
							$this->response->mensagem = $DAO->getMensagem();
						}
					}
