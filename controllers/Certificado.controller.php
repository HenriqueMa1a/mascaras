...
class CertificadoController extends Controller{
	...
	public function mostrarCertificadoSubEvento($p_Item, $p_i, $p_EventoPrincipal) { 
		...
		if($EventoParticipante->getIdStatus()==2){ // Aprovado
			$NomeStatus = '<span class="label label-success">'.$EventoParticipante->getNomeStatus().'</span>';
		} else if($EventoParticipante->getIdStatus()==3){ // Reprovado
			$NomeStatus = '<span class="label label-danger">'.$EventoParticipante->getNomeStatus().'</span>';
			...
			$NomeStatus = '<span class="label label-default">'.$EventoParticipante->getNomeStatus().'</span>';
		}
		...
		foreach ($objResumo as $itemResumo){ 
			...
			if ( ( ($itemResumo->getIdStatus()==5)||($itemResumo->getIdStatus()==6) ) && ($itemResumo->getAno()==$ano) && (strtotime(date('Y-m-d H:i:s')) > strtotime($EventoPai->getDataFinal())) ){ 
				$strCertificadoPoster[] = '<button type="button" class="btn btn-default btn-xs" onclick="visualizarCertificadoPoster(\''.$itemResumo->getChave().'\','.$itemResumo->getId().','.$ano.');" title="Visualizar Certificado Pôster"><span class="glyphicon glyphicon-file blue"></span> Certificado</button>';	
			}
		}

		if ( ($presenca) && ($status == 'aprovado') ){
			$strCertificado = '<button type="button" class="btn btn-default btn-xs" onclick="visualizarCertificado(\''.$chave.'\','.$ano.');" title="Visualizar Certificado"><span class="glyphicon glyphicon-file red"></span> Certificado</button>';
		} else{ 
			if($EventoParticipante->getIdStatus() <> 2){
				...
				if ( strtotime(date('Y-m-d H:i:s')) < strtotime($EventoPai->getDataFinal()) ) {
					$obs = 'Certificado ainda não disponível';	
				}else{
					$obs = 'Não estava presente no evento';	
				}	
			}							
		}						
	...
	public function mostrarCertificadoEvento($p_Item, $p_i) { 
		...
		if($p_Item->getIdStatus()==2){ // Aprovado
			$NomeStatus = '<span class="label label-success">'.$p_Item->getNomeStatus().'</span>';
		} else if($p_Item->getIdStatus()==3){ // Reprovado
			$NomeStatus = '<span class="label label-danger">'.$p_Item->getNomeStatus().'</span>';
			...
			$NomeStatus = '<span class="label label-default">'.$p_Item->getNomeStatus().'</span>';
		}	
		...
		if ($objPresenca){
			$strCertificado = '<button type="button" class="btn btn-default btn-xs" onclick="visualizarCertificado(\''.$objPresenca->getChave().'\','.$ano.');" title="Visualizar Certificado"><span class="glyphicon glyphicon-file red"></span> Certificado</button>';
		} else{ 
