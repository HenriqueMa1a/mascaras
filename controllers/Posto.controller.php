<?php
if(!class_exists('PostoDAO')){ require_once 'models/PostoDAO.class.php';}
...
class PostoController extends Controller{
    ...
    public function __construct(){
        parent::__construct('cadastro-posto');
        $this->Config->Debug=1;
    ...
    public function listar() {
        ...
        try {
            ...
            $DAO = new PostoDAO();
            $listagem = $DAO->listar(null, null, 'descricao', 'asc', null, null);
            if ($listagem) { 
                ...
                foreach ($listagem as $item){ 
                    $bt_view = '<a class="btn btn-vazio btn-sm btn-grid" title="Visualizar registro." onclick="visualizar('.$item->getIdPosto().');"><span class="glyphicon glyphicon-search"></span>';
					$this->response->aaData[$i] = array($bt_view, $item->getIdPosto(), $item->getDescricao(), $item->getResponsavel(), ($item->getAtivo())? 'SIM': 'NÃO');
                    $i++;
                }
    ...
    public function listarCombo() { 
	   ...
	   try {
			...
			$listagem = $DAO->listar(null, null, 'descricao', 'asc', $idPosto, $descricao);
        
			if ($listagem) {
                $this->response->records = count($listagem);
                $i = 0;
                foreach ($listagem as $item){
                    $this->response->rows[$i]['value'] = intval($item->getIdPosto()); 
   ...
   /**
    ...
    public function incluir() {
        try {
            ...
            $Responsavel = self::getVar('responsavel');
    ...
    public function editar() {
        ...
        try {
            ...
            if ($Id){
                ...
                if ($obj){
                    $obj->setDescricao($Nome);
                    $obj->setResponsavel(($Responsavel)? $Responsavel: null);
        ...
        try {
            ...
            if ($Id){
                ...
                if ($obj){
					...
					$DAO1 = new TipoUsuarioDAO();
					$DAO2 =new DistribuicaoDAO();
					$obj1 = $DAO1->listar(null, null, 'nome', null, null, null, $Id);
					$obj2 = $DAO2->listar(null, null, 'nome', null, null, $Id, null, null, null);
					
					if ($obj2) {
						
						$this->response->mensagem = "Registro não pode ser excluído. <br>";
						$this->response->mensagem .= "Existem entregas de máscaras realizadas para este posto!";

					}else { 
						if ($obj1){
							$this->response->mensagem = "Registro não pode ser excluído, existem tipos de usuários vinculados a este posto.<br>";
							$this->response->mensagem .= "Favor verificar o cadastro de Tipo de Usuários.";

						} else {	
							$this->response->Nome = $obj->getDescricao();
							if ($DAO->excluir($Id)){
								$this->response->sucesso = 1;
