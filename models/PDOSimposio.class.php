...
class PDOSimposio{
    ...
    public function getConnection(){
        try{
            ...
            //$response->notas = utf8_encode($ex->getTraceAsString());
    ...
    public function getContador($p_Chave){
        try{
            ...
            if ($this->dbType=='pgsql'){
                ...
                $p_Chave = $this->_prefixo.$p_Chave.'_seq';
                ...
                // Pegar ultimo Contador
                $stmt = $this->_conexao->prepare("SELECT contador FROM ".$this->_prefixo."_contador WHERE chave='$p_Chave' LOCK IN SHARE MODE");
                $stmt->execute();
                $rs = $stmt->fetch(PDO::FETCH_ASSOC);
                if($rs){
                    $contador = $rs['contador'];
                ...
                $stmt = $this->_conexao->prepare("INSERT INTO ".$this->_prefixo."contador (chave, contador) VALUES ('$p_Chave', (@contador:=1)) ON DUPLICATE KEY UPDATE contador = (@contador:=contador+1)");
                $stmt->execute();
                
            }else if ($this->dbType=='sqlsrv'){ 
                // Pegar ultimo Contador
                $stmt = $this->_conexao->prepare("SELECT contador FROM ".$this->_prefixo."contador WHERE chave='$p_Chave'");
                ...
                // Incrementa o Contador
                $stmt = $this->_conexao->prepare("UPDATE ".$this->_prefixo."contador SET data_acao=NOW(), acao='incrementar', id_usuario_acao=".$this->_idUsuario.", contador=$contador WHERE chave='$p_Chave'");
                //$stmt = $this->_conexao->prepare("UPDATE NextId SET Nextid=$contador WHERE name='$p_Chave'");
                $stmt->execute();
                
            }else { // MYSQL
                // Pegar ultimo Contador
                $stmt = $this->_conexao->prepare("SELECT contador FROM ".$this->_prefixo."contador WHERE chave='$p_Chave' LOCK IN SHARE MODE");
                $stmt->execute();
                $rs = $stmt->fetch(PDO::FETCH_ASSOC);
                if($rs){
                    $contador = $rs['contador'];
