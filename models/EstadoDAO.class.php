...
if(!class_exists('Estado')){ require_once 'Estado.class.php';}
/**
 * Classe DAO de acesso a dados Estado
 * 
 * @package Model.DAO
 * @category DAO
 * @since   2015-05-21
 ...
 *          #1.15
 */
class EstadoDAO extends PDOConnection {

    /**
     *  contrutor da classe EstadoDAO
     *
     ...
     *  @return void
     */    
    function __construct($p_NomeTabela='estado', $p_ChavePrimaria='id_estado', $p_NomeContador=''){
        try{
    ...
    private static function carregarModelo($p_Item){
        try{
            if ($p_Item) {
                ...
                $obj->setIdRegiao($p_Item['id_regiao']);
                $obj->setNomeRegiao(isset($p_Item['nome_regiao'])? $p_Item['nome_regiao']: '');
    ...
    public function salvar($obj){
        try{
            // Preparo a query
            $query = "INSERT INTO ".$this->_nomeTabela."  (     acao, data_acao, id_usuario_acao, id_pais, sigla_pais, id_regiao,  codigo,  sigla,  nome,  ativo, revisao)"
                                    ." VALUES ('incluir', ".$this::NOW.",  :IdUsuarioAcao, :IdPais, :SiglaPais, :IdRegiao, :Codigo, :Sigla, :Nome, :Ativo,       1)";
            $this->_query = $query;
    ...
    public function retornar($p_Id){
        try{
            ...
            if($p_Id){
                // Prepara o filtro
                if(is_numeric($p_Id)){
                    $filtro .= " AND e.id_estado=$p_Id ";
                } else {
                    $filtro .= " AND e.codigo='$p_Id' ";
                }
                
                // Preparo a query
                $query = "SELECT e.*, p.nome AS nome_pais, r.nome AS nome_regiao "
                        ." , u1.nome AS nome_usuario_acao "
                        ." FROM ".$this->_nomeTabela." e "
                        ." INNER JOIN ".$this::PREFIXO."pais p ON p.id_pais=e.id_pais "
                        ." LEFT JOIN ".$this::PREFIXO."regiao r ON r.id_regiao=e.id_regiao "
    ...
    public function atualizar($obj){
        try{
            ...
            $query = "UPDATE ".$this->_nomeTabela." "
                    ...
                    .", id_pais=:IdPais, sigla_pais=:SiglaPais, id_regiao=:IdRegiao, sigla=:Sigla, nome=:Nome, ativo=:Ativo, revisao=(revisao+1)"
                    ." WHERE id_estado=:Id";
            $this->_query = $query;
            $stmt = $this->_conexao->prepare($query);
    ...
    public function excluir($p_Id){
        try{
            // Preparo a query
            $query = "UPDATE ".$this->_nomeTabela." SET acao='excluir', data_acao=".$this::NOW.", id_usuario_acao=:IdUsuarioAcao, deletado='".$this::SIM."', codigo=null WHERE id_estado=:Id";
            $this->_query = $query;
     ...
     */
    public function listar($page=null, $rows=null, $sidx=null, $sord=null, $p_Pais=null, $p_Regiao=null, $p_Nome=null){
        try{
            $filtro='';
            if($sidx=='IdEstado'){ $sidx='id_estado';}
            ...
            if($p_Regiao){
                if(is_numeric($p_Regiao)){
                    $filtro .= " AND e.id_regiao=$p_Regiao ";
                }
            } 
            if($p_Nome) {
            ...
            $query = "SELECT e.*, p.nome nome_pais, r.nome nome_regiao "
                    ." , u1.nome AS nome_usuario_acao "
                    ." FROM ".$this->_nomeTabela." e "
                    ." INNER JOIN ".$this::PREFIXO."pais p ON p.id_pais=e.id_pais "
                    ." LEFT JOIN ".$this::PREFIXO."regiao r ON r.id_regiao=e.id_regiao "
